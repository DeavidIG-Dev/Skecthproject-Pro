...
open class ComponentMaterialDivider(context: Context, attrs: AttributeSet? = null) :
	MaterialDivider(context, attrs) {
	...
	init {
		context.withStyledAttributes(attrs, R.styleable.ComponentMaterialDivider) {
			this@ComponentMaterialDivider.setRadiusCornerSize(
				getDimension(
					R.styleable.ComponentMaterialDivider_dividerRadiusSize,
					0f
				)
			)
			this@ComponentMaterialDivider.setDividerColor(
				ColorStateList.valueOf(
					getColor(
						R.styleable.ComponentMaterialDivider_dividerColor,
						"#FFE6E0E9".toColorInt()
					)
				)
			)
	...
	override fun onDraw(canvas: Canvas) {
		...
		val right =
			if (isRtl) this.width - this.dividerInsetStart else this.width - this.dividerInsetEnd
